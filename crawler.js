...
async function main() {
  try {
    ...
    const CU_LIST = await playerSearch([825], 0); // playerSearch(시즌넘버, 최소오버롤)
    let CU_RESULTS = await playerPriceValue(CU_LIST, [1, 2, 3, 4, 5, 6, 7, 8]); // playerPriceValue(데이터 , 강화등급)
    await saveToDB(CU_RESULTS);
